...
public class StudentCompInfoController implements Initializable {

    private Stage stage;
    private AnchorPane pane;

    private StudentResultsController studentResultsController;
    ...
    public StudentCompInfoController() {

        stage = new Stage();
        studentResultsController = new StudentResultsController();
        windowMoving = new WindowMoving();
        worksheet = studentResultsController.getSelectedWorksheet();
        student = studentResultsController.getStudent();
    }

    public void createScene() throws IOException {

        stage.initStyle(StageStyle.UNDECORATED);
        FXMLLoader loader = new FXMLLoader(getClass().getResource("/view/teacher/StudentCompInfoVIEW.fxml"));
        pane = loader.load();
    ...
    private void setTable() {
        ...
        for(int i = 0; i < worksheet.getExercises().size(); i++) {
            for(int j = 0 ; j < student.getCompetitionResults().size(); j++)
            {
                if(worksheet.getExercises().get(i).getId().equals(student.getCompetitionResults().get(j).getExerciseID())
                        && worksheet.getId().equals(student.getCompetitionResults().get(j).getWorksheetID()))
                {
                    ...
                    lists.add(student.getCompetitionResults().get(j).getGotPoint());
                    times.add(student.getCompetitionResults().get(j).getGotTime());
                }
            }
        }
